...
class JobsPage extends Component {
  ...
  onGetJobDetails = async () => {
    ...
    const jwtToken = Cookies.get('jwt_token')
    const {checkboxInputs, radioInput, searchInput, checkboxLocationInputs} = this.state
    console.log(checkboxLocationInputs)
    const jobsApiUrl = `https://apis.ccbp.in/jobs?employment_type=${checkboxInputs}&location=${checkboxLocationInputs}&minimum_package=${radioInput}&search=${searchInput}`
    const optionsJobs = {
      headers: {
        Authorization: `Bearer ${jwtToken}`,
      },
  ...
  onGetInputOption = event => {
    ...
    onGetInputLocationOption = event => {
    const {checkboxLocationInputs} = this.state
    const inputNotInList = checkboxLocationInputs.filter(
      eachItem => eachItem === event.target.id,
    )
    if (inputNotInList.length === 0) {
      this.setState(
        prevState => ({
          checkboxLocationInputs: [...prevState.checkboxLocationInputs, event.target.id],
        }),
        this.onGetJobDetails,
      )
    } else {
      const filteredData = checkboxLocationInputs.filter(
        eachItem => eachItem !== event.target.id,
      )
      this.setState(
        // eslint-disable-next-line no-unused-vars
        prevState => ({checkboxLocationInputs: filteredData}),
        this.onGetJobDetails,
      )
    }
  }
  ...
  onGetCheckBoxesLocationView = () => (
    <ul className="check-boxes-container">
      {locationsOfJobs.map(eachItem => (
        <li className="li-container" key={eachItem.id}>
          <input
            className="input"
            id={eachItem.id}
            type="checkbox"
            onChange={this.onGetInputLocationOption}
          />
          <label className="label" htmlFor={eachItem.id}>
            {eachItem.location}
          </label>
        </li>
      ))}
